...
public final class FSAVE32 extends Instruction {
    
    private final Segment m_segment;
    private final Pointer m_offset;
    
    public FSAVE32(Intel80386 cpu,
                   Segment segment,
                   Pointer offset) {
    ...
    public void run() {
        ...
        m_cpu.writeMEM16(m_segment, addr + 4, m_cpu.FPU.getStatus());
        m_cpu.writeMEM16(m_segment, addr + 8, m_cpu.FPU.getTags());
        
        // Store stack
        addr += 28;
        for(int i = 0; i < 8; i++, addr += 10) {
            
            Float80 fp80 = ExtendedDouble.toFloat80(m_cpu.FPU.getST(i));
